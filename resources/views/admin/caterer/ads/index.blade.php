...
{{-- Caterer Identity Banner --}}
<div class="alert alert-info d-flex align-items-center gap-3 mb-4">
    <i data-feather="briefcase" style="width:28px;height:28px;flex-shrink:0"></i>
    <div>
        <strong style="font-size:1.1rem">{{ $caterer->BusinessName }}</strong>
        @if($caterer->Description)
            <div class="text-muted small mt-1">{{ $caterer->Description }}</div>
        @endif
        <div class="mt-1">
            <span class="badge bg-{{ $caterer->IsActive ? 'success' : 'secondary' }}">
...
<div class="card-header">
    <strong><i data-feather="speaker" style="width:16px" class="me-1"></i>Submit New Advertisement for <span class="text-primary">{{ $caterer->BusinessName }}</span></strong>
</div>
<div class="card-body">
<form method="POST" action="{{ route('caterer.ads.store') }}" class="row g-3" id="adForm" enctype="multipart/form-data">
@csrf
<div class="col-md-3">
    <label class="form-label fw-semibold">Ad Title *</label>
    <input name="title" class="form-control" placeholder="e.g. Ramadan Special Catering" required>
</div>
<div class="col-md-3">
...
<div class="card-header d-flex align-items-center justify-content-between">
    <strong>Submitted Advertisements — {{ $caterer->BusinessName }}</strong>
...
<tr>
    ...
    <td>
        <span class="fw-semibold text-primary">{{ $caterer->BusinessName }}</span>
    </td>
    <td>{{ $ad->Title }}</td>
...
<tr><td colspan="8" class="text-center text-muted py-5">
    ...
    No advertisements submitted yet for {{ $caterer->BusinessName }}.
</td></tr>
@endforelse
</tbody>
</table>
