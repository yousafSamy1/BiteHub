...
<div class="page-content">
    <div class="d-flex justify-content-between align-items-center flex-wrap grid-margin mb-3">
        <div>
            <h4 class="mb-1">Request: {{ $item ? $item->ItemName : 'Special Service' }}</h4>
            <p class="text-muted">Chatting with <strong>{{ $customerUser->name }}</strong> about a customization request.</p>
        </div>
        <a href="{{ route('caterer.customization.requests') }}" class="btn btn-outline-primary btn-sm">
        ...
        <div class="col-md-12">
            <div class="card border-0 shadow-sm" style="border-radius: 20px; overflow: hidden; display: flex; flex-direction: column; min-height: 500px; max-height: 70vh;">
                ...
                <div class="card-footer bg-white p-3 border-top">
                    <form id="chatForm" action="{{ route('caterer.preorder.chat.send', [$menuItemId ?? 0, $customerUser->UserID]) }}" method="POST">
                        @csrf
                    ...
                    <div class="mt-3 d-flex gap-2">
                        ...
                        <form action="{{ route('caterer.chat.reject', optional($lastReq)->LiveChatID ?? 0) }}" method="POST">
                            @csrf
...
<div class="modal fade" id="approveModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <form action="{{ route('caterer.chat.approve', $lastRequest->LiveChatID) }}" method="POST">
            @csrf
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Final approval for request: <strong>{{ $item ? $item->ItemName : 'Special Custom Service' }}</strong></p>
                    <div class="mb-3">
