...
<div class="card-body">
<form method="POST" action="{{ route('caterer.menu.store') }}" class="row g-3" enctype="multipart/form-data">
@csrf
<div class="col-md-3">
...
<thead class="table-light">
    <tr><th>#</th><th>Image</th><th>Name</th><th>Category</th><th>Description</th><th>Price</th><th>Status</th><th>Actions</th></tr>
</thead>
<tbody>
...
<tr>
    ...
    <td>
        @if($item->images->count() > 0)
            <img src="{{ asset('upload/item_images/'.$item->images->first()->Image) }}" style="width:50px;height:50px;object-fit:cover;border-radius:4px;">
        @else
            <span class="text-muted" style="font-size:12px">No Image</span>
        @endif
    </td>
    <td><strong>{{ $item->ItemName }}</strong></td>
    <td class="text-muted" style="font-size:.85rem">{{ $item->category->Name ?? '—' }}</td>
    <td class="text-muted" style="font-size:.85rem">{{ Str::limit($item->Description, 40) }}</td>
    <td>
    ...
    <td>
        {{-- Toggle --}}
        <form method="POST" action="{{ route('caterer.menu.toggle', $item->MenuItemID) }}" class="d-inline">@csrf
        ...
        {{-- Delete --}}
        <form method="POST" action="{{ route('caterer.menu.delete', $item->MenuItemID) }}" class="d-inline" onsubmit="return confirm('Delete this item?')">
            @csrf @method('DELETE')
...
<form method="POST" action="{{ route('caterer.menu.update', $item->MenuItemID) }}" enctype="multipart/form-data">@csrf
<div class="modal-body">
    ...
    <div class="mb-3"><label class="form-label">Category</label>
        <select name="CategoryID" class="form-select" onchange="toggleNewCat(this,'new-cat-{{ $item->MenuItemID }}')">
            <option value="">None</option>
            @foreach($categories as $cat)
            <option value="{{ $cat->CategoryID }}" @selected($item->CategoryID == $cat->CategoryID)>{{ $cat->Name }}</option>
            @endforeach
            <option value="__new__">➕ Add New Category</option>
        </select>
        <input type="text" id="new-cat-{{ $item->MenuItemID }}" name="NewCategoryName" class="form-control mt-2 d-none" placeholder="New category name...">
    </div>
    <div class="mb-3">
    ...
    <div class="mb-3">
        ...
        <div class="d-flex flex-wrap gap-2">
            @foreach($item->images as $img)
                <img src="{{ asset('upload/item_images/'.$img->Image) }}" style="width:60px;height:60px;object-fit:cover;border-radius:4px;border:1px solid #ddd;">
            @endforeach
        </div>
    </div>
    @endif
...
@empty
<tr><td colspan="8" class="text-center text-muted py-4">No menu items yet. Add your first item above!</td></tr>
@endforelse
</tbody>
</table>
</div>
...
function toggleNewCat(select, inputId) {
    if (select.value === '__new__') {
        window.location.href = "{{ route('caterer.categories') }}";
    } else {
