...
<div class="promo-card">
    <div class="promo-header">
        ...
        <div style="display:flex; gap:10px; align-items:center;">
            <form method="GET" action="{{ route('admin.promo_codes') }}" style="display:flex; align-items:center; gap:8px;">
                <select name="creator_role" class="promo-input" onchange="this.form.submit()" style="padding: 8px 12px; width:auto; border-radius:10px;">
                    <option value="">All Creators</option>
                    <option value="Admin" {{ request('creator_role') == 'Admin' ? 'selected' : '' }}>Admin</option>
                    <option value="KitchenOwner" {{ request('creator_role') == 'KitchenOwner' ? 'selected' : '' }}>Kitchen Owner</option>
                    <option value="Caterer" {{ request('creator_role') == 'Caterer' ? 'selected' : '' }}>Caterer</option>
                </select>
            </form>
                ...
                <tr>
                    ...
                    <td style="text-align:right;">
                        <div class="d-flex gap-2 justify-content-end">
                            ...
                            {{-- Toggle --}}
                            <form method="POST" action="{{ route('admin.promo_codes.toggle', $promo->PromoCodeID) }}" style="display:inline;">
                                @csrf
                            ...
                            @endif
                            {{-- Delete --}}
                            <form method="POST" action="{{ route('admin.promo_codes.delete', $promo->PromoCodeID) }}" style="display:inline;" onsubmit="return confirm('Delete this promo code?')">
                                @csrf @method('DELETE')
                ...
                <div class="modal fade modal-promo" id="editModal{{ $promo->PromoCodeID }}" tabindex="-1">
                    <div class="modal-dialog modal-dialog-centered modal-lg">
                        <div class="modal-content">
                            ...
                            <form method="POST" action="{{ route('admin.promo_codes.update', $promo->PromoCodeID) }}">
                                @csrf
...
function executeAnnounce(id, btn) {
    ...
    fetch(`/admin/promo-codes/${id}/announce`, {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            // Update button to "Sent" state permanently
